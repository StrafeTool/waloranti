...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ...
    ImRect bb(ImVec2(0, 0), window->DC.CursorPos + size);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f - 5));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        //window->DrawList->AddRectFilled(grab_bb.Min - ImVec2(1, 1), grab_bb.Max + ImVec2(1, 1), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, 1), grab_bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(0.f, 1.f, 0.706f, 1.f)), GetColorU32(ImVec4(0.f, 1.f, 0.529f, 1.f)), GetColorU32(ImVec4(0.f, 1.f, 0.529f, 1.f)), GetColorU32(ImVec4(0.f, 1.f, 0.706f, 1.f)));
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    if (g.LogEnabled)
    ...
    RenderTextClipped(grab_bb.Min - ImVec2(1, 2), grab_bb.Max + ImVec2(1, 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    ImGui::PopStyleColor();
